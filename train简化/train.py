    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:#进度条
        for iteration, batch in enumerate(gen):#enunmerate为可遍历对象创建索引序列
            if iteration >= epoch_size:#iteration迭代器
                break
            images, targets = batch[0], batch[1]#batch0为图片,batch1为标签
            with torch.no_grad():
                if cuda:
                    images = Variable(torch.from_numpy(images).type(torch.FloatTensor)).cuda()#数组读入图片，并转换为张量
                    targets = [Variable(torch.from_numpy(ann).type(torch.FloatTensor)) for ann in targets]#获得标签信息
                else:
            ...
            for i in range(3):
                loss_item, num_pos = yolo_losses[i](outputs[i], targets)#i对应输出的特征层
...
#    writer=SummaryWriter(log_dir='view',comment='Linear')
    ...
    if True:
        ...
        if Use_Data_Loader:
            ...
            val_dataset = YoloDataset(lines[num_train:], (Config["img_h"], Config["img_w"]), False)
            gen = DataLoader(train_dataset, shuffle=True, batch_size=Batch_size, num_workers=4, pin_memory=True,
                                    drop_last=True, collate_fn=yolo_dataset_collate)#组合数据集和采样器；将自定义的Dataset根据batch size大小、是否shuffle等封装成一个Batch Size大小的Tensor，用于后面的训练
            gen_val = DataLoader(val_dataset, shuffle=True, batch_size=Batch_size, num_workers=4,pin_memory=True, 
                                    drop_last=True, collate_fn=yolo_dataset_collate)
        else:
            gen = Generator(Batch_size, lines[:num_train],
