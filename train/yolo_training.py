...
#     center_distance=(((box_a[:, 0]+box_a[:, 2]/2).unsqueeze(1)-(box_b[:, 0]+box_b[:, 2]/2).unsqueeze(0))**2)+(((box_a[:, 1]+box_a[:, 3]/2).unsqueeze(1)-(box_b[:, 1]+box_b[:, 3]/2).unsqueeze(0))**2)
#     length_x=torch.max(box_a[:, 0].unsqueeze(1),box_b[:, 0].unsqueeze(0))-torch.min(box_a[:, 0].unsqueeze(1),box_b[:,0].unsqueeze(0))
#     length_y=torch.max(box_a[:, 1].unsqueeze(1),box_b[:, 1].unsqueeze(0))-torch.min(box_a[:, 1].unsqueeze(1),box_b[:,1].unsqueeze(0))
#     length=(length_x)**2+(length_y)**2
#     iou=inter / union
#     diou=inter / union-center_distance/length
#     return diou  # [A,B]
...
#anchor框，shape（9，2），array([[116,  90],[156, 198],[373, 326],[ 30,  61],[ 62,  45],[ 59, 119],[ 10,  13],[ 16,  30],[ 33,  23]])
        self.num_anchors = len(anchors)#获取anchors长度，即数量：9
        self.num_classes = num_classes#种类数：7
        self.bbox_attrs = 5 + num_classes#4+1+num_classes,4为中心点位置+预测框长宽，1为中心点是否在框内

        self.feature_length = [img_size[0]//32,img_size[0]//16,img_size[0]//8]#特征层尺寸[13,26,52]
        self.img_size = img_size

        self.ignore_threshold = 0.5#iou值
        self.lambda_xy = 1.0
        self.lambda_wh = 1.0
    ...
    def forward(self, input, targets=None):
        ...
        bs = input.size(0)#图片数目

        in_h = input.size(2)#特征层高

        in_w = input.size(3)#特征层宽
        ...
        #-----------------------------------------------------------------------#
        stride_h = self.img_size[1] / in_h#计算一个特征点对应的像素
        stride_w = self.img_size[0] / in_w

        #-------------------------------------------------#
        #   此时获得的scaled_anchors大小是相对于特征层的
        #-------------------------------------------------#
        scaled_anchors = [(a_w / stride_w, a_h / stride_h) for a_w, a_h in self.anchors]#获得scaled_anchors相对于特征层的大小
        ...
        prediction = input.view(bs, int(self.num_anchors/3),
                                self.bbox_attrs, in_h, in_w).permute(0, 1, 3, 4, 2).contiguous()#bbox_attrs总参数数量，permute改变排列顺序，tensor使变量在内存中连续分布
