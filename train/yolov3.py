...
class YoloBody(nn.Module):
    ...
    def forward(self, x):
        def _branch(last_layer, layer_in):
            for i, a in enumerate(last_layer):
                #print(e)
                #print(i)
                layer_in = a(layer_in)
                #print(e(layer_in).shape)
                #print(layer_in.shape)
                if i == 4:#取第五次卷积的结果，之后进行下采样
                    out_branch = layer_in
            return layer_in, out_branch#layer_in为DBL，一次CONV, out_branch为第五次卷积后的结果

        x2, x1, x0 = self.backbone(x)#获得三个特征层输出
