...
if __name__ == "__main__":
    ...
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if np.shape(model_dict[k]) ==  np.shape(v)}#读取预训练权重
    ...
    with open(annotation_path,encoding='utf-8') as f:
        lines = f.readlines()#读取txt所有信息
    np.random.seed(10101)
    np.random.shuffle(lines)#重排
    np.random.seed(None)
    num_val = int(len(lines)*val_split)#计算验证集数
    num_train = len(lines) - num_val#计算训练集数9-1   
    ...
    if True:
        ...
        for param in model.backbone.parameters():#读取模型参数
